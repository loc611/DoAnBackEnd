    ...
    const fetchEligibleStudents = async () => {
        try {
            ...
            const eligible = res.data
                ...
                .map(u => ({
                    _id: u.profile._id, // Notice we need Student document _id, wait, the API expects Student _id or User _id?
                    // userController creates Student with userId.
                    // When we add students to class, we update Student collection. So we need Student _id.
                    studentId: u.profile._id,
...
const ClassDetails = () => {
    ...
    const handleRemoveStudent = async (student) => {
        ...
        if (result.isConfirmed) {
            try {
                await api.delete(`/classes/${id}/students/${student._id}`);
                        ...
                        <table className="w-full text-left text-sm text-gray-600">
                            ...
                            <tbody>
                                {students.length === 0 ? (
                                    ...
                                    <tr key={s._id} className="hover:bg-blue-50/50 transition-colors border-b border-gray-50 last:border-0">
